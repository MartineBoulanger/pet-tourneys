export const LoginFormSkeleton = () => {
  return (
    <div className='bg-light-grey shadow-md rounded-lg p-5'>
      <div className='bg-dark-grey shadow-md rounded-lg p-5'>
        <div className='w-full flex flex-col gap-8'>
          <div className='animate-pulse w-full p-2 border rounded-md bg-foreground h-10'></div>
          <div className='animate-pulse w-full p-2 border rounded-md bg-foreground h-10'></div>
          <div className='animate-pulse w-full p-2 rounded-lg bg-blue h-10'></div>
        </div>
      </div>
      <div className='animate-pulse w-full p-2 rounded-md bg-medium-grey h-5 mt-4'></div>
      <div className='animate-pulse w-full p-2 rounded-md bg-medium-grey h-5 mt-2'></div>
    </div>
  );
};

export const SignUpFormSkeleton = () => {
  return (
    <div className='bg-light-grey shadow-md rounded-lg p-5'>
      <div className='bg-dark-grey shadow-md rounded-lg p-5'>
        <div className='w-full flex flex-col gap-8'>
          <div className='animate-pulse w-full p-2 border rounded-md bg-foreground h-10'></div>
          <div className='animate-pulse w-full p-2 border rounded-md bg-foreground h-10'></div>
          <div className='animate-pulse w-full p-2 border rounded-md bg-foreground h-10'></div>
          <div className='animate-pulse w-full p-2 rounded-lg bg-blue h-10'></div>
        </div>
      </div>
      <div className='animate-pulse w-full p-2 rounded-md bg-medium-grey h-5 mt-4'></div>
    </div>
  );
};

export const ForgotResetFormSkeleton = () => {
  return (
    <div className='bg-light-grey shadow-md rounded-lg p-5'>
      <div className='bg-dark-grey shadow-md rounded-lg p-5'>
        <div className='w-full flex flex-col gap-8'>
          <div className='animate-pulse w-full p-2 border rounded-md bg-foreground h-10'></div>
          <div className='animate-pulse w-full p-2 rounded-lg bg-blue h-10'></div>
        </div>
      </div>
    </div>
  );
};

export const TournamentFormSkeleton = () => {
  return (
    <div className='bg-light-grey shadow-md rounded-lg p-5'>
      <div className='bg-dark-grey shadow-md rounded-lg p-5'>
        <div className='w-full flex flex-col gap-8'>
          <div className='animate-pulse w-full p-2 border rounded-md bg-foreground h-10'></div>
          <div className='animate-pulse w-full p-2 border rounded-md bg-foreground h-10'></div>
          <div className='animate-pulse w-full p-2 border rounded-md bg-foreground h-10'></div>
          <div className='animate-pulse w-full p-2 border rounded-md bg-foreground h-10'></div>
          <div className='flex gap-5'>
            <div className='animate-pulse w-full p-2 rounded-lg bg-red h-10'></div>
            <div className='animate-pulse w-full p-2 rounded-lg bg-blue h-10'></div>
          </div>
        </div>
      </div>
    </div>
  );
};

export const UploadFormSkeleton = () => {
  return (
    <div className='bg-light-grey shadow-md rounded-lg p-5'>
      <div className='bg-dark-grey shadow-md rounded-lg p-5'>
        <div className='w-full flex flex-col gap-8'>
          <div className='flex gap-5'>
            <div className='animate-pulse w-full p-2 border rounded-md bg-foreground h-10'></div>
            <div className='animate-pulse w-full p-2 border rounded-md bg-foreground h-10'></div>
          </div>
          <div className='flex gap-5'>
            <div className='animate-pulse w-full p-2 border rounded-md bg-foreground h-10'></div>
            <div className='animate-pulse w-full p-2 border rounded-md bg-foreground h-10'></div>
          </div>
          <div className='flex gap-5'>
            <div className='animate-pulse w-full p-2 border rounded-md bg-foreground h-10'></div>
            <div className='animate-pulse w-full p-2 border rounded-md bg-foreground h-10'></div>
          </div>
          <div className='animate-pulse w-full p-2 border rounded-md bg-foreground h-30'></div>
          <div className='animate-pulse w-full p-2 border rounded-md bg-foreground h-15'></div>
          <div className='flex gap-5'>
            <div className='animate-pulse w-full p-2 rounded-lg bg-red h-10'></div>
            <div className='animate-pulse w-full p-2 rounded-lg bg-blue h-10'></div>
          </div>
        </div>
      </div>
    </div>
  );
};

export const ChartSkeleton = () => {
  return (
    <div className='bg-dark-grey shadow-md rounded-lg p-5'>
      <div className='animate-pulse w-full p-2 rounded-lg bg-gray-500 h-[410px]'></div>
    </div>
  );
};
