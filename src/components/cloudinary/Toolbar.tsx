...
import { searchImagesPerFolder } from '@/actions/cloudinary/searchImages';
import { Button, Input } from '@/components/ui';
import { ToolbarProps } from '@/types/cloudinary.types';
...
export function Toolbar({
  ...
  const handleSearch = async (term: string) => {
    ...
    if (term.trim() === '') {
      const result = await getMultipleImages(folder, undefined, 20);
      if (result.success) {
    ...
    const result = await searchImagesPerFolder(term, folder);
    if (result.success) {
    ...
    <div className='bg-background rounded-lg shadow-md p-2.5 lg:p-5 mb-2.5 lg:mb-5'>
      ...
      {images.length > 0 ? (
        <div className='mt-2.5 lg:mt-5 flex items-center gap-4'>
          <span className='text-sm text-foreground/70'>{`Showing ${images.length} images`}</span>
          {selected.length === 0 && (
            <Button
