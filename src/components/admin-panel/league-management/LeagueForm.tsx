...
import { TournamentFormSkeleton } from '@/components/layout/Skeletons';
import { createLeague } from '@/actions/supabase/api-schema/leagues/createLeague';
import { updateLeague } from '@/actions/supabase/api-schema/leagues/updateLeague';
import { LeagueFormProps } from '@/types/components.types';

export function LeagueForm({ initialData = null }: LeagueFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (initialData?.id) {
        const { success } = await updateLeague(initialData.id, submissionData);
        if (!success) {
          toastError('Failed to update the league');
        }
        toastSuccess('League has been successfully updated');
      } else {
        const { success } = await createLeague(submissionData);
        if (!success) {
          toastError('Failed to create the league.');
        }
        toastSuccess('League has been successfully created');
      }

      router.push('/admin-panel/leagues');
    } catch (error) {
      console.error(error);
  ...
  return (
    <div className='bg-light-grey shadow-md rounded-lg p-2.5 lg:p-5'>
      <Form
        ...
        handleClick={() => router.push('/admin-panel/leagues')}
      >
        <Input
          type='text'
          id='name'
