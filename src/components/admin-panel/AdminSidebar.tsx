...
import {
  ...
  pagesManagementData,
} from '@/lib/navigation-data/admin';
import { Button, Heading } from '@/components/ui';
import { AdminLink } from './AdminLink';
import { AdminSidebarProps } from '@/types/components.types';
...
export const AdminSidebar = ({
  ...
  const filteredPagesManagementData = pagesManagementData.filter(
    ({ roleAllowed }) => {
      if (roleAllowed === 'both') return true;
      if (roleAllowed === 'admin') return isAdmin;
      if (roleAllowed === 'author') return isAuthor;
          ...
          <nav className='flex-1 p-5 overflow-y-auto'>
            <div className='space-y-5'>
              ...
              <div className='space-y-2.5'>
                ...
                {filteredPagesManagementData.map(
                  ({ linkText, imageSrc, id, url }) => (
                    <AdminLink
                      key={id}
                      onClose={closeSidebar}
