...
export async function getLeaguePetStats(id: string): Promise<LeaguePetStat[]> {
  const pu = await apiTable('pet_usage', id);
  const m = await apiTable('matches', id);
  const bl = await apiTable('battle_logs', id);

  const { data: matches } = await m
    .select('id, player1, player2')
    .eq('is_forfeit', false);

  const { data: records, error } = await pu.select('*');
  const { data: allBattleLogs, error: battleLogsError } = await bl.select('*');

  if (error || battleLogsError) throw error || battleLogsError;
  if (!records || !allBattleLogs || !matches) return [];

  const petMap = new Map<string, LeaguePetStat>();
